...
# =============================================================================
# REINSTALL SCRIPT - Skips database setup (preserves existing database)
# Use this when your game is broken but you want to keep your database data
# =============================================================================
...
# Database credentials (used for config files only, NOT for DB setup)
DB_NAME=fvdb
DB_USER=farmer
DB_PASS=examplePassword

echo "=== Reinstalling FarmVille Replowed (preserving database) ==="

# Install dependencies (skip mariadb-server since DB already exists)
apt-get update
apt install -y apache2 php php-xml php-dom npm composer php-mysql libgtk2.0-0 libdbus-glib-1-2

# Backup existing .env if it exists
if [ -f "$FVRP_DIR/.env" ]; then
    echo "Backing up existing .env..."
    cp "$FVRP_DIR/.env" /tmp/.env.backup
fi
...
# Restore .env backup or create new one
...
# Run migrations WITHOUT seeding (preserves existing data)
echo "Running migrations (without seed)..."
php artisan migrate

# Update AMFPHP credentials
sed -i.bak \
...
ln -sf $BASILISK_DIR/basilisk /usr/bin/basilisk
ln -sf $BASILISK_DIR/browser/chrome/icons/default/default16.png /usr/share/icons/hicolor/16x16/apps/basilisk.png
ln -sf $BASILISK_DIR/browser/chrome/icons/default/default32.png /usr/share/icons/hicolor/32x32/apps/basilisk.png
ln -sf $BASILISK_DIR/browser/chrome/icons/default/default48.png /usr/share/icons/hicolor/48x48/apps/basilisk.png
ln -sf $BASILISK_DIR/browser/icons/mozicon128.png /usr/share/icons/hicolor/128x128/apps/basilisk.png
...
# Download and run dehasher for assets
